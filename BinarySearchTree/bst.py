...
class BST:
    ...
    def __repr__ (self):
        ...
        print("The program reaches render")
        g1.render('img/g1')
        return("hello")
...
class BSTNode:
    ...
    def rep (self):
        print("The program reachs __repr__")
        if (self.left):
            g1.node(chr(self.left.key+48))
            g1.edge(chr(self.key+48), chr(self.left.key+48))
            self.left.rep()
        if (self.right):
            g1.node(chr(self.right.key+48))
            g1.edge(chr(self.key+48), chr(self.right.key+48))
            self.right.rep()
